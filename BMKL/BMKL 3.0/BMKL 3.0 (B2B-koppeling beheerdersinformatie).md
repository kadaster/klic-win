...
Dit document biedt een handleiding voor het als decentrale netbeheerder aanleveren van beheerdersinformatie via de B2B-koppeling volgens BMKL 3.0. \
...
---------------------------------------------------------
## Verschil BMKL 2.1 en BMKL 3.0
De aanleiding van het wijzigen van het BMKL is de beoogde upgrade van de KLIC-standaarden.  \
Veel wijzigingen die in scope zijn van de upgrade hebben betrekking op  de netinformatie die in ZIP-bestanden wordt uitgewisseld. Doordat het uitwisselen van ZIP-bestanden het zelfde proces blijft volgen, heeft dat niet direct impact op de *berichten* die uitgewisseld worden in het BMKL.  \
De issues die impact hebben het het `GebiedsInformatieAanvraag`-object, leiden wel direct tot een aanpassing in het BMKL.  \
...
[issue #333](https://github.com/Geonovum/imkl2015-review/issues/333) heeft betrekking op het aantal toegestande tekens voor de klantreferentie. Dat heeft geen invloed op het modelschema van het `GebiedsInformatieAanvraag`-object, maar heeft mogelijk wel voor de consumerende partij impact.
...
| :information_source: De endpoints voor BMKL 3.0 zijn nog niet vastgesteld; in onderstaande documentatie staan daarom de BMKL 2.1 endpoints genoemd. |
|:------|

Voor het oriënteren, plannen en uitvoeren van graafwerkzaamheden in een bepaald gebied hebben grondroerders informatie nodig over de locatie en aard van de in de grond aanwezige kabels en leidingen.  \
...
Voor de beschrijving van functionaliteit die in het portaal van de Externe Test Omgeving (ETO) worden aangeboden, wordt verwezen naar [Externe Test Omgeving (ETO)](../../Externe%20Test%20Omgeving%20(ETO)/). \
Hierin wordt ook beschreven hoe een testmelding voor netbeheerders kan worden opgevoerd. \
...
Om dit zipbestand aan te kunnen leveren, moet er eerst een 'decentrale netbeheerder testmelding' worden opgevoerd (zie [Externe Test Omgeving (ETO)](..\..\Externe%20Test%20Omgeving%20(ETO)/README.md#testmelding-voor-netbeheerder)). Daarmee wordt er een gebiedsinformatie-aanvraag aangemaakt waarvoor beheerdersinformatie en de bijbehorende documenten aangeleverd kunnen worden.
...
De centrale netbeheerder actualiseert in de ETO-omgeving eerst (indien van toepassing) documenten, en vervolgens netinformatie. \
Daarna kan ook de centrale netbeheerder een testmelding opvoeren (zie [Externe Test Omgeving (ETO)](..\..\Externe%20Test%20Omgeving%20(ETO)/README.md#testmelding-voor-netbeheerder)).
...
![procesmodel](bijlagen/8.3.3.2-Produceren-volgens-BMKL-decentraal.png "Procesmodel BMKL (decentrale netbeheerder)")
...
### Procesmodel BMKL (centrale netbeheerder)

![procesmodel](bijlagen/8.3.3.3-Produceren-volgens-BMKL-centraal.png "Procesmodel BMKL  (centrale netbeheerder)")
_Figuur 2 Procesmodel (centrale netbeheerder)_
...
- Het zipbestand bevat exact één bestand met de extentie `.xml`. Dit bestand bevat de beheerdersinformatie in [IMKL 3.0 formaat](https://docs.geostandaarden.nl/kl/def-st-imkl-20250618/).
...
- De naam van het zipbestand moet eindigen op '_V3.zip'.
...
De documentatie over de werking van deze interfaces is beschikbaar in de vorm van [Swagger](http://swagger.io) specificatie. Deze documentatie is te vinden op: https://service10.kadaster.nl/klic/api-docs/ Selecteer in het dropdownbox de juiste API met de juiste versie.  \
_Merk op dat de Swagger specificatie van Versie 3 nog niet beschikbaar is_.
...
| :information_source: De endpoints voor BMKL 3.0 zijn nog niet vastgesteld. De bovenstaande pagina wordt t.z.t aangevuld.  <br> In onderstaande documentatie staan daarom de BMKL 2.1 endpoints genoemd.|
|:------|

De endpoints die in onderstaande voorbeelden worden gebruikt, zijn relatief ten opzichte van deze basispaden.  \
In de voorbeelden wordt uitgegaan van de API's op de externe test-omgeving.
...
De API Documentatie is beschikbaar in de vorm van [Swagger](http://swagger.io) specificatie. Deze documentatie is te vinden op: https://service10.kadaster.nl/klic/api-docs/ Selecteer in het dropdownbox de juiste API met de juiste versie.  \
_Merk op dat de Swagger specificatie van Versie 3 nog niet beschikbaar is_.
 
Indien in de dropdownbox wordt gekozen voor beheerderinformatie, worden de services voor het afhandelen van beheerdersinformatie-aanvragen weergegeven.  \
Let wel, niet alle services zijn geautoriseerd om door netbeheerders gebruikt te worden.

![mijnKadaster](bijlagen/Swagger-beheerdersinformatie.png "Swagger API documentatie beheerdersinformatie")
_Figuur 3 API Documentatie Beheerdersinformatie (BMKL)_
[README.md](README.md)
...
In de Swagger-documentatie worden de verschillende parameters per API weergegeven.  \
...
![Swagger](bijlagen/Swagger-Fragment-Gebiedsinformatieaanvragen-Datum-selectie.png "Swagger - Selectie Parameters (fragment gebiedsinformatieAanvragen)")
...
{
    "giAanvraagId": "4c8353bd-3907-40ee-84b0-5f54ac38d4d1",
    "ordernummer": "3015000924",
    "positienummer": "00010",
    "klicMeldnummer": "26G000649",
    "aanvrager":{
        "contactpersoon": {
            "naam":"Aanvrager01",
            "telefoon":"0881235648",
    ...
    "referentie":"referentieveld mag vanaf BMKL 3.0 max. 60 tekens lang zijn",
    "opdrachtgever":{
        "contactpersoon":{
            "naam":"Kadaster",
    ...
    "aanvraagDatum":"2026-11-03T10:38:14+01:00",
    "mutatieDatum":"2026-11-03T10:38:14.451+01:00",
    "giAanvraagStatus": "https://api.klic.kadaster.nl/waardelijsten/v2/giAanvraagStatussen/giOpen",
    "werkzaamhedenInfo": {
        "soortWerkzaamheden":[
          "http://definities.geostandaarden.nl/imkl2015/id/waardelijst/SoortWerkzaamhedenValue/rioolwerkzaamheden",
          "http://definities.geostandaarden.nl/imkl2015/id/waardelijst/SoortWerkzaamhedenValue/onderzoekenInDeBodem",
          "http://definities.geostandaarden.nl/imkl2015/id/waardelijst/SoortWerkzaamhedenValue/verhardingswerk"
        ],
       "methode": [
          "http://definities.geostandaarden.nl/imkl2015/id/waardelijst/GraafmethodeValue/mechanischBorstelen",
          "http://definities.geostandaarden.nl/imkl2015/id/waardelijst/GraafmethodeValue/sleufloosGestuurdeBoring"
        ],
        "maximaleWerkdiepte": "http://definities.geostandaarden.nl/imkl2015/id/waardelijst/MaximaleWerkdiepteValue/80cm",
      },
    "omschrijvingWerkzaamheden": "betonnen rioolbuis vervangen",
...
curl https://service10.kadaster.nl/klic/ntd/bmkl/v2/gebiedsinformatieAanvragen/4c8353bd-3907-40ee-84b0-5f54ac38d4d1/beheerdersinformatieAanvragen/330d0526-0586-4843-ad86-04d8969fc768/aanleveringen
 ...
 -H 'Authorization: Bearer 1d021976-91c8-4b46-ab9b-529088d0f3de'
 -H 'Accept: application/json'
 -F "netinformatie=@C:/Aanleveringen/Decentraal/nbact1/BMKL_DECENTRAAL_V3.zip"
```
